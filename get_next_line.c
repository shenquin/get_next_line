...
/*   Updated: 2020/10/02 10:41:55 by henquinseba      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

/* cut join_buff before bsn */
...
/* cut join_buff after bsn */

static char				*ft_stock(char *join_buf)
{
	unsigned int	i;
	char			*line_to_stock;
...
int						get_next_line(int fd, char **line)
{
	...
	if (str)
		if (!(join_buf = ft_strdup(str)))
			return (ft_free(join_buf, -1));
	while (!ft_check_bsn(join_buf) && (i = read(fd, buf, BUFFER_SIZE)) > 0)
	{
		buf[i] = '\0';
